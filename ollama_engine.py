...
def _fallback_qa(q: str, data: dict, relevant_sections: list[tuple[str, str, float]] | None = None) -> str:
    ...
    # PRIORITY: Handle dataset/training questions FIRST (before generic method handling)
    # This catches questions like "how many images are used to train the model?"
    dataset_keywords = ["dataset", "data", "training", "test", "image", "sample", "size", "train", "val", "validation", "augmentation"]
    is_dataset_query = any(kw in ql for kw in dataset_keywords)
    
    if is_dataset_query:
        # Search ALL sections comprehensively for dataset/training info
        all_sections = list(s.keys())
        numeric_findings = []
        ...
        for sec_name in all_sections:
            ...
            for sent in sentences:
                ...
                if len(sent) < 20 or len(sent) > 300:
                    continue
                # Must have a number to be relevant for dataset questions
                ...
                # High score for dataset-specific keywords in the sentence
                if any(kw in sent_lower for kw in ['image', 'sample', 'training', 'test', 'dataset', 'train', 'val', 'data', 'augment']):
                    score += 10
                # Match with query keywords
                if any(kw in ql for kw in ['train', 'image', 'sample', 'dataset', 'data', 'test', 'augment']):
                    if any(kw in sent_lower for kw in ['train', 'image', 'sample', 'dataset', 'data', 'test', 'augment']):
                        score += 7
                # Bonus for sentences with actual numbers
                if re.search(r'\d+', sent):
                    score += 3
                # Bonus for phrases like "X images", "X samples", "X datasets"
                if re.search(r'\d+\s*(images?|samples?|datasets?|training|test|examples?|instances?)', sent_lower):
                    score += 5
                
                if score > 5:  # Higher threshold for better quality
        ...
        if numeric_findings:
            result_lines = []
            seen = set()
            for score, sec, sent in numeric_findings[:6]:
                ...
                if key not in seen:
                    ...
                    seen.add(key)
            if result_lines:
                return "**Dataset & Training Information:**\n" + "\n".join(result_lines)
    
    # Handle other numeric/specific queries (percentages, accuracy, numbers)
    ...
    if is_numeric_query and not is_dataset_query:
